# FILE: Restore-Sourcetree-Settings.ps1
# This script will restore the Sourcetree settings to a specified
...
# ---------------------------------------------------------------
Write-Header "RESTORE SOURCETREE SETTINGS"
...
# ---------------------------------------------------------------
# Perform restore of Sourcetree settings
# ---------------------------------------------------------------
function Copy-FileWithCheck($source, $destination) {
    if (Test-Path $source) {
        Copy-Item -Path $source -Destination $destination -Force
        Write-Message "Restored $(Split-Path $source -Leaf)"
    } else {
        Write-Warning "Source file not found: $source. Skipping."
    }
}
$SourcetreeSettingsFiles | ForEach-Object {
    Copy-FileWithCheck -source (Join-Path $SourcetreeSettingsBackupPath $_) -destination $SourcetreeSettingsLocalPath
}
